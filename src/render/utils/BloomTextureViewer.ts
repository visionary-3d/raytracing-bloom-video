...
class BloomTextureViewer {
  ...
  initGui() {
    ...
    viewerFolder
      .add(guiControls, 'texture', ['prefilter', 'ping', 'pong', 'down'])
      .onChange(() => {
  ...
  changeBindGroup() {
    ...
    this.uniformBuffer.uniforms.mul.set(
      Math.pow(2, guiControls.mipLevel) * multiplier
    );
  ...
  render() {
    ...
    Object.values(this.renderPassDescriptor.colorAttachments)[0].view =
